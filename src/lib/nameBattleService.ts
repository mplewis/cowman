 ...
 */
export async function getNameBattleStats(guildId: string, limit = 10) {
	const client = db
 ...
 */
export async function getBattleHistoryWithRenameLinks(guildId: string, limit = 20) {
	const client = db
	...
	log.debug({ guildId, battleCount: results.length }, 'Retrieved battle history with rename links')
